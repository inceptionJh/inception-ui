...
const Test = () => {
  ...
  const yMax = d3.max(data.map((v) => v.price));
  const yDiff = yMax - 0;
  const yGap = Math.round(yDiff * 0.1 / Math.pow(10, `${yDiff}`.length - 2)) * Math.pow(10, `${yDiff}`.length - 2);
  const yDomain = [yMax + yGap, 0] as [number, number];
  const yRange = [padding.top, 500 - padding.bottom];

  const yTicks = React.useMemo(() => {
    const ticks = [] as number[];
    for (let tick = 0; tick <= yDomain[0];) {
      ticks.push(tick);
      tick += yGap;
    }
    return ticks;
